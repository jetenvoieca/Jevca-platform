...
export default function PageEditor({
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <Link href={`/sites/${siteId}`} className="text-sm text-neutral-500 hover:underline">
        ← Back to Web Site
      </Link>
        ...
        {blocks.map((block, i) => (
          <div key={block.id} className="rounded-lg border border-neutral-200 p-4">
                  ...
                  <div className="mb-2 grid grid-cols-4 gap-2">
                    {block.images.map((img) => (
                      <div key={img.imageId} className="group relative">
                        <img src={img.url} alt="" className="h-16 w-full rounded object-cover" />
                        <button
                          type="button"
                          onClick={() =>
            ...
            {block.type === "artwork" && (
              <div>
                ...
                <p className="mt-1 text-xs text-neutral-400">
                  This block always shows the artwork&apos;s current title, image, price and
                  status live — editing the artwork updates every page it appears on.
                </p>
              </div>
            )}
            ...
            {block.type === "video" && (
              <div>
                {block.url && (
                  <video src={block.url} controls className="mb-2 max-h-48 w-full rounded-md" />
                )}
                <MediaPicker
                  siteId={siteId}
                  mode="single"
